...
@section('breadcrumb')
<li class="breadcrumb-item">Teams</a></li>
@endsection
@section('content')
<x-flash-message></x-flash-message>
<a href="{{ route('user.teams.create') }}" class="btn-primary">create team</a>
...
<form action="{{ URL::current() }}" method="get" class="row q-3 mt-3 mb-3">
    ...
    onclick="location.href='{{ route('user.teams.index') }}'"
>
    Reset
</button>
</form>
        ...
        <tbody>
            @foreach ($members as $item)
                <tr>
                    ...
                    <td><a href="{{ route('user.teams.edit',$item->id) }}" class="btn-primary">Edit</a></td>
                    <td>
                        <form action="{{ route('user.teams.destroy',$item->id) }}" method="post">
                            @csrf
                            @method('DELETE')
    ...
    @push('scripts')
        <script>
            document.getElementById('resetBtn').addEventListener('click', function() {
                window.location.href = "{{ route('user.teams.index') }}";
            });
        </script>